...
var listSchema = mongoose.Schema({name: String, userId: Number, complete: Boolean, completedAt: Date})
var ToDoList = mongoose.model('ToDoList', listSchema)

function createDefaultList() {
    var defaultToDoList = new ToDoList({name: "TESTING FOO BAR", userId: 1, complete: false, completedAt: null})// {name: "Item Two", id: 2, complete: false, completedAt: null}, {name: "Item Three", id: 3, complete: true, completedAt: '2017-07-06T16:43:41Z'})
    defaultToDoList.save(function(err, callback){
        if(err) return console.error(err)
        console.log('saved default list')
    })
}
